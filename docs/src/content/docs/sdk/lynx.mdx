...
The Pulsar Lynx SDK (`lynx-pulsar`) exposes haptic feedback through three building blocks: `Presets`, `usePatternComposer`, and `useRealtimeComposer`.

All methods are callable from the **ReactLynx background thread** — i.e. `bindtap` / `bindtouch*` handlers, `useEffect`, async callbacks. The native module is not bound on the main thread (snapshot evaluation), so calling a Pulsar API at the top of a render expression silently no-ops. See [How it works → threading](/pulsar/lynx/architecture/#a-note-on-threading) for the full story.

## Requirements

* ReactLynx 0.118+ (`@lynx-js/react`) — declared as a peer dependency
* A Lynx host that registers `PulsarLynxModule` (the included [Lynx Pulsar host](/pulsar/lynx/architecture/) does this for you)
* iOS only at present — Android wiring is in place in the native module but not yet shipped through the Lynx host

## Installation

Latest version: **`0.1.0`** (early preview — API surface matches `react-native-pulsar` but is not yet pinned).
...
The package ships unbundled TypeScript (`main: src/index.tsx`) — your Lynx bundler (rspeedy) compiles it as part of your app. Native sources for iOS (`PulsarLynxModule.{h,m}` + the Pulsar Swift SDK) and Android (`PulsarLynxModule.kt`) ship under the package's `ios/` and `android/` directories — they still need to be wired into your Lynx host's native build. The included [Lynx Pulsar host](/pulsar/lynx/architecture/) does this end-to-end; the [run the demo](/pulsar/lynx/build-prompt/) walkthrough gets you to a working iPhone install in about 30 minutes the first time.

## Presets

A collection of ready-to-use haptic patterns. Every preset is a function on the `Presets` object.
...
Cross-platform haptics that map to native OS generators. Use these when you want the OS-default feel rather than a Pulsar pattern.

```tsx
import { Presets } from 'lynx-pulsar';

<view bindtap={() => Presets.System.impactMedium()}>
  <text>Impact</text>
</view>
```
...
| Method | Description |
|---|---|
| `Presets.System.impactHeavy()` | `UIImpactFeedbackGenerator.heavy` |
| `Presets.System.impactLight()` | `UIImpactFeedbackGenerator.light` |
| `Presets.System.impactMedium()` | `UIImpactFeedbackGenerator.medium` |
| `Presets.System.impactRigid()` | `UIImpactFeedbackGenerator.rigid` |
| `Presets.System.impactSoft()` | `UIImpactFeedbackGenerator.soft` |
| `Presets.System.notificationError()` | `UINotificationFeedbackGenerator.error` |
| `Presets.System.notificationSuccess()` | `UINotificationFeedbackGenerator.success` |
| `Presets.System.notificationWarning()` | `UINotificationFeedbackGenerator.warning` |
| `Presets.System.selection()` | `UISelectionFeedbackGenerator.selection` |

### System presets (Android)

Android-only constants under `Presets.System.Android.*`, mapping to `HapticFeedbackConstants` and `VibrationEffect`. The JS shape exists on every platform — on iOS, calls reach the Swift SDK and silently no-op since no such preset is registered. They become live on a Lynx Android host.
...
A ReactLynx hook for composing and playing a custom `Pattern`. The pattern is parsed on mount and whenever it changes. Native resources are released automatically on unmount.

### Parameters

**`pattern`** (optional) — Type: `Pattern`

A haptic pattern to parse on mount. When provided, the pattern is parsed automatically and re-parsed whenever the value changes.

### Returns

- **`play()`** — Plays the parsed pattern.
- **`stop()`** — Stops the active pattern.
- **`parse(pattern: Pattern)`** — Parses and replaces the current pattern.
- **`isParsed()`** — Returns `true` if a pattern has been parsed and is ready to play.
...
function MyComponent() {
  ...
  return (
    <view bindtap={() => composer.play()}>
      <text>Play pattern</text>
    </view>
  );
}
```
...
A ReactLynx hook for real-time haptic control. Provides live amplitude and frequency modulation, useful for gesture-driven or continuously evolving haptic experiences. Haptics stop automatically on unmount.
...
- **`set(amplitude: number, frequency: number)`** — Updates the ongoing haptic with new amplitude and frequency values. Safe to call at touch-event rate.
- **`playDiscrete(amplitude: number, frequency: number)`** — Plays a single discrete haptic event.
- **`stop()`** — Stops the active haptic.
- **`isActive()`** — Returns `true` if a haptic is currently playing.
...
function MyComponent() {
  ...
  return (
    <view
      bindtouchstart={() => realtime.set(0.3, 0.5)}
      bindtouchmove={(e) => {
        const amp = clamp(e.detail.y / 800, 0, 1);
        realtime.set(amp, 0.5);
      }}
      bindtouchend={() => realtime.stop()}
    />
  );
}
```
...
Global configuration. `Settings` is an object of methods, not a hook — call it from anywhere on the background thread (event handlers, effects).

The three `_(Android only)_` methods below are present on the `Settings` object on every platform — calling them on iOS is safe (no-op or ignored). They're shipped as part of the surface so app code can stay platform-agnostic once Lynx adds Android.
...
| `Settings.forceHapticsSupportLevel(level)` | _(Android only)_ Override the detected support level. Intended for testing/debugging fallback behavior. |
| `Settings.enableImpulseCompositionMode(state: boolean)` | _(Android only)_ Enable or disable impulse composition mode. |
| `Settings.setRealtimeComposerStrategy(strategy)` | _(Android only)_ Set the strategy used by the realtime composer. |

```tsx
import { Settings } from 'lynx-pulsar';
...
The Lynx adapter exposes a 5-level support scale (vs. 4 in the React Native adapter). `MINIMAL_SUPPORT` is the extra rung — a device that can vibrate but lacks the primitive APIs needed for ticks. On Android, `Settings.forceHapticsSupportLevel(level)` uses these exact numeric enum values when selecting the fallback path.

```tsx
import { Settings, HapticSupport, Presets } from 'lynx-pulsar';
...
```ts
import { RealtimeComposerStrategy } from 'lynx-pulsar';
...
```tsx
import { Settings, RealtimeComposerStrategy } from 'lynx-pulsar';
